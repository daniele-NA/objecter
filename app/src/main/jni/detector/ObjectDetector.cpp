...
void ObjectDetector::initDetectionModel(const char* modelBuffer, int size) {
	...
	if (m_input_tensor->dims->data[0] != 1 ||
		...
		LOG_E("Detection model must have input dims of 1x%ix%ix%i", DETECTION_MODEL_SIZE,
	...
	if (TfLiteInterpreterGetOutputTensorCount(m_interpreter) != 4) {
		LOG_E("Detection model graph needs to have 4 and only 4 outputs!");
		return;
	}
...
DetectResult* ObjectDetector::detect(Mat src) {
	auto* res = new DetectResult[DETECT_NUM];
	if (m_model == nullptr) {
		return res;
	}
	...
	if (TfLiteInterpreterInvoke(m_interpreter) != kTfLiteOk) {
		LOG_E("Error invoking detection model");
		return res;
	}
